...
Three subclasses (GrassCell, SandCell, WaterCell) extend Cell. This lets me reuse code while giving each terrain its own rules and graphics. For example, Grass is simple to walk on, Sand has a higher movement cost, and Water blocks movement.
...
**Generics**
The Grid<T extends Cell> class uses generics so that the grid can hold any subtype of Cell. This improves type safety and reusability. For example, the same grid code works whether it contains only GrassCell or a mixture of all three cell types.
...
GrassCell.java, SandCell.java, WaterCell.java – subclasses of Cell with their own behaviour and drawing.

Walkable.java, Swimmable.java – interfaces for movement rules.
